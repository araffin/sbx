...
class TQCrossQPolicy(BaseJaxPolicy):
    ...
    def build(self, key: jax.Array, lr_schedule: Schedule, qf_learning_rate: float) -> jax.Array:
        key, actor_key, qf1_key, qf2_key, bn_key = jax.random.split(key, 5)
        key, bn_key1, bn_key2 = jax.random.split(key, 3)
        ...
        qf1_params = self.qf.init(
            {"params": qf2_key, "dropout": dropout_key1, "batch_stats": bn_key1},
            obs,
            action,
            train=False,
        )
        qf2_params = self.qf.init(
            {"params": qf2_key, "dropout": dropout_key1, "batch_stats": bn_key2},
            obs,
            action,
            train=False,
        )

        self.qf1_state = BatchNormTrainState.create(
            apply_fn=self.qf.apply,
            params=qf1_params["params"],
            batch_stats=qf1_params["batch_stats"],
            tx=self.optimizer_class(
                learning_rate=qf_learning_rate,  # type: ignore[call-arg]
                **self.optimizer_kwargs,
            ),
        )
        self.qf2_state = BatchNormTrainState.create(
            apply_fn=self.qf.apply,
            params=qf2_params["params"],
            batch_stats=qf2_params["batch_stats"],
            tx=self.optimizer_class(
                learning_rate=qf_learning_rate,  # type: ignore[call-arg]
                **self.optimizer_kwargs,
            ),
