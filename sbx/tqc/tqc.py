    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            if self.num_timesteps > learning_starts and True and self.num_timesteps % 2 == 0:
                ...
                # shape: (n_envs, n_quantiles)
                qf2_quantiles = self.policy.qf.apply(
                    self.policy.qf2_state.params,
                    self._last_obs,
                    actions,
                    # rngs={"dropout": dropout_key_1},
                )
                qf1_quantiles = jnp.expand_dims(qf1_quantiles, axis=1)
                qf2_quantiles = jnp.expand_dims(qf2_quantiles, axis=1)
